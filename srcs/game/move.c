...
int is_valid_king_move(t_gui *gui, t_case *start_square, t_case *end_square)
{
    ...
    int is_white = start_square->status & WHITE;
        ...
        else
        {
                ...
                gui->case_list[61].status == EMPTY && gui->case_list[62].status == EMPTY)
            {
                return 4; // Roque côté roi pour les noirs
                     ...
                     gui->case_list[57].status == EMPTY && gui->case_list[58].status == EMPTY && gui->case_list[59].status == EMPTY)
            {
...
static void move(t_gui *gui, t_case *start_square, t_case *end_square)
{
...
static int move_is_valid(t_gui *gui, t_case *start_square, t_case *end_square)
{
	...
	{
		if (!is_valid_king_move(gui, start_square, end_square))
			return 0;
	}
	glutPostRedisplay();
	return 1;
